...
def get_user_catalog_pane(root, frame2):
    ...
    text_box = Text(bottom_pane, height=32, width=70, bg='gray75', yscrollcommand=True)
    text_box.grid(column=0, row=0, padx=148, pady=(11, 10))
...
def initialize_user_edit_pane(modify_pane):
    ...
    add_tool_button = Button(modify_pane,
                             text="Add tool",
                             command=lambda: add_tool(name_text, desc_text, sharable_option))
    ...
    barcode_text.grid(column=1, row=5, pady=(10, 0))
    delete_tool_button = Button(modify_pane,
                                text="delete tool",
                                command=lambda: delete_tool(barcode_text))
    ...
    barcode_label = Label(modify_pane, text="Enter a barcode: ", bg='white')
    barcode_label.grid(column=0, row=7, padx=5, pady=(10, 0))
    barcode_text = Text(modify_pane, width=20, height=1, bg='white')
    barcode_text.grid(column=1, row=7, pady=(10, 0))
    ...
    modify_tool_button = Button(modify_pane,
                                text="modify tool",
                                command=lambda: modify_tool(barcode_text, what_to_modify_option, modify_text.get(0)))
    modify_tool_button.grid(column=2, row=8, pady=(10, 0), padx=(10, 0))



    initialize_modify_action(modify_clicked, modify_text)


def initialize_modify_action(modify_clicked, modify_text):
...
def add_tool(name_text, desc_text, sharable_option):
...
def modify_tool(barcode_text, what_to_modify_option, modify_string):
    print("barcode: " + barcode_text + " what to modify: " + what_to_modify_option + " modify: " + modify_string)
